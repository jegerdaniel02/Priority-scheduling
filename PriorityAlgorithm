...
time=1
maxtmp=0
for nums in range(0,highestpriority+1):
    
    for eachrow in allrows:
        if eachrow[3]==nums:
            rowssorted.append(eachrow)
            #satte rowsa i rowssorted i stigende rekkefølge etter kolonneD

def findLowestPriority(time):
    global maxtmp
    global gtime
    ...
    for i in range(0,maxtmp+1):
        for row in temp:
            if row[3]==i:
                ...
                #sortert tempsorted etter kolonneD av de som har arrived.
...
#    print("len of tmpsortd",len(tempsorted))   
    if len(tempsorted) >0:
        ...
        #currentPID er nå av de som arriva, den med lavest priority.
        #print(tempsorted)
        #temp=[]
